...
def main():
  if os.path.exists("listaDomande.ts"):
    os.remove("listaDomande.ts")

  sys.stdout = open("listaDomande.ts", "w")
  print('export const LESSONS = {')
  for count in range(1, 1000):
    generateDomanda(count)
  print('}')
...
def generateDomanda(id):
  ...
  my_dict = {
    'dataDomanda':  random.choice(randomAnnoInvio),
    'statoDomanda': 'inviata',
    'id': id,
    'courseId': 1,
    'spid': {
      'codiceFiscale': spid.codiceFiscale,
      'nome': spid.nome,
      'cognome': spid.cognome,
      'dataNascita': spid.dataNascita,
      'luogoDiNascita': spid.luogoDiNascita,
      'residenza': spid.residenza,
      'via': spid.via,
      'telefono': spid.telefono,
      'email': spid.email,
      'domDigitale': spid.domDigitale,
    }, 'istruzione': {
      'istitutoFrequentato': istruzione.istitutoFrequentato,
      'tipoDiploma': istruzione.tipoDiploma,
      'annoDiploma': istruzione.annoDiploma,
      'provinciaIstituto': istruzione.provinciaIstituto,
      'comuneIstituto': istruzione.comuneIstituto,
      'sedeIstituto': istruzione.sedeIstituto,
    },
    'linguaStraniera': {
      'lingua': linguaSelezionata,
      'id': idLingua,
    },
    'titoliPreferenziali': titoloSelezionato,
    'figliACarico': {
      'haFigli': aventeFigli,
      'numeroFigli': numFigli,
    },
    'riserve': random.choice(randomRiserve),
    'categorieProtette': {
      'isCategorieProtette': catProtette.isCategorieProtette,
      'percInvalidita': catProtette.percInvalidita,
      'dataCertificazione': catProtette.dataCertificazione,
      'enteRilascioCertificato': catProtette.enteRilascioCertificato,
      'ausilioProva': catProtette.ausilioProva,
      'tempiAggiuntivi': catProtette.tempiAggiuntivi,
      'esenzioneProvaSelettiva': catProtette.esenzioneProvaSelettiva,
    }
  }
