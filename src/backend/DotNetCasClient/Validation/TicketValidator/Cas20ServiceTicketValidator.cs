    ...
    class Cas20ServiceTicketValidator : AbstractCasProtocolTicketValidator
    {
        ...
        protected override ICasPrincipal ParseResponseFromServer(string response, string ticket)
        {
            ...
            if (serviceResponse.IsAuthenticationSuccess)
            {
                ...
                {
                    AuthenticationAttributes authenticationAttribute = new AuthenticationAttributes();
                    IDictionary<string, IList<string>> attributesDictionary = authenticationAttribute.AttributesDictionaryFromXml(response);
                    return new CasPrincipal(new Assertion(authSuccessResponse.User, attributesDictionary), proxyGrantingTicketIou, authSuccessResponse.Proxies);
                } 
                else
                {
                    AuthenticationAttributes authenticationAttribute = new AuthenticationAttributes();
                    IDictionary<string, IList<string>> attributesDictionary = authenticationAttribute.AttributesDictionaryFromXml(response);
                    return new CasPrincipal(new Assertion(authSuccessResponse.User, attributesDictionary), proxyGrantingTicketIou);
                }
            }
